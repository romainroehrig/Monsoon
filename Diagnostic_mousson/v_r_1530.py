...
data_nc='C:/Users/Ludovic/Desktop/PFE/Simu_WA/DELANNOY_slab_cont_WA_arpsfx_monthly_'+str(var)+'_1850-1855.nc'
file = nc.Dataset(data_nc)
bounds2 =[ i for i in np.arange(0,104,4)]
unit='%'
cmapref = cm.RdBu
conv=1
ext='both'
...
lev= file.variables['pstd'][:]
#dates = nc.num2date(file['time'][:],units=(file['time']).units,calendar=(file['time']).calendar)
data = (shiftgrid(180,file.variables[str(var)][:,:,:,:]*100,nc.Dataset(data_nc).variables['lon'][:],start=False,cyclic=360.0))[0]
lon=(shiftgrid(180,file.variables[str(var)][:,:,:,:]*100,nc.Dataset(data_nc).variables['lon'][:],start=False,cyclic=360.0))[1]
data=np.average(data[5:8:12],axis=0)


# Average between 10W and 10E
# select longitude indices between -10 and 10. All parentheses are important
lon_inds = np.where((lon > 0) & (lon < 15))[0]
# final average
data_1 = np.ma.average(data[:,:,lon_inds],axis=2)
...
c1 = ax.contour(xaxis,yaxis,data_2, bounds1, levels=range(0,5,1), colors='black',extend=ext,linewidths=2.0,linestyles='solid')
c2 = ax.contour(xaxis,yaxis,data_2, bounds1, levels=range(-5,-1,1),colors='black', extend=ext,linewidths=2.0,linestyles='dotted')
