...
data = file.variables[var][:,k,:,:]*conv
lon=(shiftgrid(180,file.variables[str(var)][:,k,:,:]*conv,nc.Dataset(data_nc).variables['lon'][:],start=False,cyclic=360.0))[1]
data = (shiftgrid(180,file.variables[str(var)][:,k,:,:]*conv,nc.Dataset(data_nc).variables['lon'][:],start=False,cyclic=360.0))[0]
...
plt.title('spinup_'+str(var)+str(level)+str(cont)+'_'+str(sim))
plt.savefig('Spinup/spinup_'+str(var)+str(level)+str(cont)+'_'+str(sim)+'.png')
plt.show()
