...
bounds1 =[ i for i in np.arange(-30,23,3)]
unit='(m/s)'
conv=1
ext='both'
...
lon=(shiftgrid(180,file.variables[str(var)][:,:,:,:]*conv,nc.Dataset(data_nc).variables['lon'][:],start=False,cyclic=360.0))[1]
inds = [d.month in seasons['JJAS'] for d in dates]
data_1=np.average(data[inds],axis=0)
...
lon_inds = np.ma.where((lon > 0) & (lon < 15))[0]
# final average
data = np.ma.average(data_1[:,:,lon_inds],axis=2)

lat_inds = np.ma.where((lat > -35) & (lat < 35))[0]
...
c2 = ax.contour(xaxis,yaxis,data_2, bounds1, levels=range(-21,3,3),colors='black', extend=ext,linewidths=2.0,linestyles='dotted')

norm = mpl.colors.BoundaryNorm(bounds1, cmap.N)
yaxis = file.variables[str(lv)][:]*0.01
c = ax.contourf(xaxis,yaxis,data_1 , bounds2, cmap=cmap, norm=norm2, extend=ext)
ax.clabel(c1, fontsize=10)
...
plt.title('u_t_015_'+str(sim))
plt.savefig('images/u_t/u_t_015_'+str(sim)+'.png')
plt.show()
