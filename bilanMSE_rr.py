...
for var in var2read:
    ...
    if var == 'ps':
        data[var],lon = shifgrid(180,fnc[var][:,:,:],fnc['lon'][:],start=False,cycle=360.)
    else:
        lev = fnc['lev']
        data[var],lon = shifgrid(180,fnc[var][:,:,:,:],fnc['lon'][:],start=False,cycle=360.)

    dates = nc.num2date(fnc[t][:],units=fnc[t].units,calendar=fnc[t].calendar)

    inds = [d.month in seasons['JJAS'] for d in dates]
    # Average over JJAS
    data[var] = np.ma.average(data[inds],axis=0)
...
for ilat in range(0,nlat):
    for ilon in range(0,nlon-1):
        dx[:,ilat,ilon] = dlon[ilon]*math.cos(lat[ilat])
...
int_mse_advu[:,:] = mse_advu[0,:,:]*(ps[:,:]-(lev[0]+lev[1])/2.)/g
int_mse_advv[:,:] = mse_advv[0,:,:]*(ps[:,:]-(lev[0]+lev[1])/2.)/g
int_mse_advw[:,:] = mse_advw[0,:,:]*(ps[:,:]-(lev[0]+lev[1])/2.)/g


for ilev in range(1,nlev-1):
    int_mse_advu[:,:] = int_mse_advu[:,:] + mse_advu[ilev,:,:]*(lev[ilev-1]-lev[ilev+1])/2./g
    int_mse_advv[:,:] = int_mse_advv[:,:] + mse_advv[ilev,:,:]*(lev[ilev-1]-lev[ilev+1])/2./g
    int_mse_advw[:,:] = int_mse_advw[:,:] + mse_advw[ilev,:,:]*(lev[ilev-1]-lev[ilev+1])/2./g

int_mse_advu = int_mse_advu/ps
int_mse_advv = int_mse_advv/ps
int_mse_advw = int_mse_advw/ps

int_mse_advhor = adv_mse_advu + adv_mse_advv

# Saving data
fout = nc.Dataset('output.nc','w')
...
levAxis[:] = lev[:]
latAxis = fout.createVariable('lat','f4',('lev',))
latAxis[:] = lat[:]
lonAxis = fout.createVariable('lon','f4',('lev',))
lonAxis[:] = lon[:]
