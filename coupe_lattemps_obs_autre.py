...
if var=='pr':
	...
	bounds =[ i for i in np.arange(285,316,2)]
	unit='(K)'
	cmapref = cm.coolwarm
	ext='both'
elif var=='w':	
	bounds =[ i for i in np.arange(-0.1,0.11,0.01)]
	unit='(Pa/s)'
	cmapref = cm.seismic
	ext='both'
elif var=='r':	
...
# final average
data = np.ma.average(data[:,:,:,lon_inds],axis=3)
#Time_average

data_new=np.zeros((12,128))
for i in range(0,12):
	data_new[i,:]=np.average(data[range(0+i,396+i,12),2,:],axis=0)
...
plt.title('ty_10W10E_850_'+str(var))
plt.xlabel('Month')
plt.ylabel('Latitude')
plt.savefig('Coupe_lattemps_obs/'+str(var)+'/ty_10W10E_'+str(var)+'850.png')
plt.show()
