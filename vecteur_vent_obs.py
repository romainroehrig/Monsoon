...
bounds =[ i for i in np.arange(0,25,1)]
cs=ax.contourf((shiftgrid(180,file.variables[str(var)][:,:,:]*24,nc.Dataset(data_nc).variables['lon'][:],start=False,cyclic=360.0))[1],lat, data_p[0,:,:], bounds, cmap=cmap, norm=norm, extend=str(ext), transform=ccrs.PlateCarree())
q = ax.quiver(lon[80:165],lat[56:120],U, V)
cbar = fig.colorbar(cs, shrink=0.9, orientation='horizontal', pad=0.1, extend='max', boundaries=bounds, ticks=bounds , label=str(unit))
plt.title('vecteur_vent_pr_JJAS')
plt.savefig('vecteur_vent.png')
plt.show()
